...
- **删除连接超时失败**：修复删除数据库连接时因尝试连接数据库导致的 `connect ETIMEDOUT` 错误。删除连接记录时不再需要连接数据库，直接操作本地存储即可。

## [0.4.6]

### 修复

- **构建流程**：修复 GitHub Actions 发布工作流配置问题。

### 内部

- **SQLite3 运行时复制脚本**：新增 `scripts/copy-sqlite3-runtime.cjs`，用于在打包时正确复制 `sqlite3` 原生模块运行时文件，确保扩展发布后 SQLite 功能可用。
- **忽略文件优化**：更新 `.gitignore` 与 `.vscodeignore`，精简打包排除规则，避免遗漏或冗余忽略。
