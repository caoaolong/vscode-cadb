...
const mockData = {
  fields: [
    { id: 'field-name', name: 'name', type: 'varchar', length: 255, defaultValue: '', nullable: false },
    { id: 'field-email', name: 'email', type: 'varchar', length: 255, defaultValue: '', nullable: false },
    { id: 'field-age', name: 'age', type: 'int', length: null, defaultValue: '0', nullable: true }
  ],
  indexes: [
    { id: 'index-primary', name: 'PRIMARY', type: 'primary', fields: ['id'], unique: true },
    { id: 'index-unique-email', name: 'unique_email', type: 'unique', fields: ['email'], unique: true }
  ]
};
...
layui.use(['element', 'form', 'layer'], function() {
  ...
  function loadFieldForm(field) {
      ...
      <form class="layui-form" lay-filter="field-form">
        <div class="layui-form-item">
          ...
          <div class="layui-input-block">
            <input type="text" name="name" value="${field.name}" placeholder="字段名" 
        ...
        <div class="layui-form-item">
          ...
          <div class="layui-input-block">
            <input type="text" name="defaultValue" value="${field.defaultValue}" 
    ...
    $('#btn-delete-field').on('click', function() {
      layer.confirm('确定要删除字段 "' + field.name + '" 吗？', {
        icon: 3,
        title: '确认删除'
      }, function(index) {
  ...
  function loadIndexForm(index) {
    ...
    const formHtml = `
      <form class="layui-form" lay-filter="index-form">
        <div class="layui-form-item">
          ...
          <div class="layui-input-block">
            <input type="text" name="name" value="${index.name}" placeholder="索引名" 
                   class="layui-input" required lay-verify="required">
          </div>
        </div>
        ...
        <div class="layui-form-item">
          ...
          <div class="layui-input-block">
            <select name="type" lay-verify="required">
              <option value="primary" ${index.type === 'primary' ? 'selected' : ''}>主键索引</option>
              <option value="unique" ${index.type === 'unique' ? 'selected' : ''}>唯一索引</option>
              <option value="normal" ${index.type === 'normal' ? 'selected' : ''}>普通索引</option>
    ...
    $('#btn-delete-index').on('click', function() {
      layer.confirm('确定要删除索引 "' + index.name + '" 吗？', {
        icon: 3,
        title: '确认删除'
      }, function(idx) {
  ...
  form.on('submit(save-index)', function(data) {
    ...
    data.field.fields = fieldsStr.split(',').map(f => f.trim()).filter(f => f);
